      ...
      {showCoffee && (
        <>
          <a
            ...
            className="inline-flex items-center gap-1.5 text-sm font-medium text-muted hover:text-amber-500 transition-colors"
          >
          ...
          {links.length > 0 && <span className="text-faint2 select-none">·</span>}
        </>
      )}
      {links.map((link, i) => (
        <span key={link.href} className="flex items-center gap-3">
          {i > 0 && <span className="text-faint2 select-none">·</span>}
          <a
            href={link.href}
