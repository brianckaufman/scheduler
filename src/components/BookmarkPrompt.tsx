...
export default function BookmarkPrompt({ eventSlug }: BookmarkPromptProps) {
  ...
  return (
    <div className="animate-fade-in mb-4 bg-social-50 dark:bg-[#1C1939] border border-social-200 dark:border-[#2B1169] rounded-2xl p-4">
      <div className="flex items-start gap-3">
        <div className="shrink-0 mt-0.5">
          <svg
            className="w-5 h-5 text-social-fg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-social-fg">{title}</p>
          <p className="text-xs text-secondary mt-0.5">{desc}</p>
          <div className="flex items-center gap-3 mt-2">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-social-fg bg-social-100 dark:bg-[#1C1939] px-2.5 py-1 rounded-full">
              {isMobile ? (
            ...
            <button
              ...
              className="text-xs text-secondary hover:text-body transition-colors cursor-pointer"
            >
              {dismissLabel}
            </button>
          </div>
